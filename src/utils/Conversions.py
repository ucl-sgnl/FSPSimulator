...
def fit_tle_to_spacecraft_states(spacecraft_states: ArrayList, satellite_number: int, classification: str,
    ...
    tle_builder = TLEPropagatorBuilder(tle_first_guess, PositionAngle.MEAN, 100.0) #the 1000 here is the position scale. i.e. the factor by which the "real" orbital parameters are scaled down to produce normalized parameters.
    fitter = FiniteDifferencePropagatorConverter(tle_builder, threshold, 10000) # the 10000 here is the max number of iterations to reach threshold
