...
def fit_tle_to_spacecraft_states(spacecraft_states: ArrayList, satellite_number: int, classification: str,
    ...
    tle_first_guess = TLE(satellite_number,
                          ...
                          revolution_number,
                          b_star_first_guess)
    threshold = 10.0 #distnace threshold in meters between the spacecraft state and the TLE
    print("tle_first_guess:", tle_first_guess)
    tle_builder = TLEPropagatorBuilder(tle_first_guess, PositionAngle.TRUE, 1.0) #the 1000 here is the position scale. i.e. the factor by which the "real" orbital parameters are scaled down to produce normalized parameters.
