...
from fspsim.utils.SpaceCatalogue import SpaceCatalogue
...
def propagate_satellite_profiling(args):
    ...
    directory = 'src/fspsim/tests/profiling_results/'
...
def run_parallel_sim(settings):
    ...
    if settings["repull_catalogues"] and os.path.exists(get_path('src/fspsim/data/catalogue/All_catalogue_latest.csv')):
        ...
        dump_pickle('src/fspsim/data/catalogue/SATCAT_before_prop.pickle', SATCAT)

    else:
        SATCAT.Catalogue = load_pickle('src/fspsim/data/catalogue/SATCAT_before_prop.pickle')

    if settings["environment"] == "development" and os.path.exists(get_path(f'src/fspsim/data/catalogue/SATCAT_before_prop_{scenario_name}.pickle')):
        SATCAT = load_pickle(f'src/fspsim/data/catalogue/SATCAT_before_prop_{scenario_name}.pickle')
    else:
        if settings["scenario_name"] != "baseline":
            print("Creating Launch Model...")
            launch_file_object = Prediction2SpaceObjects('src/fspsim/data/prediction_csv/FSP_Predictions.csv', 'src/fspsim/data/prediction_csv/sim_settings.json')
    ...
    dump_pickle(f'src/fspsim/data/catalogue/prop_{scenario_name}.pickle', SATCAT)

    print(f"Output: {get_path(f'src/fspsim/data/catalogue/{scenario_name}.pickle')}")
    print(f"Number of Satellites: {len(SATCAT.Catalogue)}")
    print("Simulation Complete")

if __name__ == '__main__':
    ##### Profiling #####
    with open('src/fspsim/tests/profiling_results/profiling_main.txt', 'w') as f:
        sys.stdout = f  # redirect output to file
        policy = json.load(open(get_path('src/fspsim/data/prediction_csv/sim_settings.json'), 'r'))
        profiler = cProfile.Profile()
        profiler.enable()
