...
class TestMyClass(unittest.TestCase):
    ...
    def setUp(self):
        with open(os.path.join(os.getcwd(), 'src/fspsim/data/prediction_csv/sim_settings.json'), 'r') as f:
            json_data = f.read()
    ...
    def test_CreateCatalogueActive(self):
        ...
        self.assertTrue(os.path.exists(os.path.join(os.getcwd(), 'src/fspsim/data/catalogue/active_jsr_spacetrack_latest.csv')))
       
        # Test if the file contains data
        df = pd.read_csv(os.path.join(os.getcwd(), 'src/fspsim/data/catalogue/active_jsr_spacetrack_latest.csv'))
    ...
    def test_CreateCatalogueAll(self):
        ...
        self.assertTrue(os.path.exists(os.path.join(os.getcwd(), 'src/fspsim/data/catalogue/All_catalogue_latest.csv')))
        # Test if the file contains data
        df = pd.read_csv(os.path.join(os.getcwd(), 'src/fspsim/data/catalogue/All_catalogue_latest.csv'))
        self.assertGreater(len(df), 0)
        # Additional test cases:
        # Test if the exported file has the expected columns
        # Test if the data in the exported file is as expected
